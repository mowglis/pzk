...
QR_dir = "/".join([home_dir, "scan_files"])
file_type = "jpeg"

f_ascii = "/".join([home_dir, "ascii.table"])

def make_ascii_dict():
    """ make dict with ascii code chars"""
    ascii = {}
    f = open(f_ascii)
    for line in f:
...
def write_rec(uchazec, pzk_c, scan_file):
    ...
    rec = {}
    dict2rec = [('prijmeni','PR'),('jmeno','JM'),('pohlavi','PH'),('rc','RC'),('e_mail0','EM'),('misto_nar','misto_nar'),('cizinec','statni_obc'),('izo_zs','IZO'),('p1','pr_2'),('p2','pr_3'),('poradi_zajmu','skola_1'),('zast_prijmeni','prijmeni'),('zast_jmeno','jmeno'),('zast_pohlavi','ph'),('e_mail1','em')]
    fields = ()
    vals = ()
...
if __name__ == "__main__":
    ...
    for f in sorted(QR_files, key=get_num_file):
        print("File: {}".format(os.path.basename(f)), end="")
        cmd = "/usr/bin/zbarimg -q {} 2> /dev/null".format(f)
        stdout = os.popen(cmd)
        qr = ""
